...
### Шаг 3: Автоматическое чанкирование и инициализация баз данных

> **Внимание: Все базы данных и векторный поиск инициализируются АВТОМАТИЧЕСКИ!**
> - **Инициализация БД**: Docker контейнеры автоматически применяют `init_postgres.sql` для PostgreSQL, `init_clickhouse.xml` для ClickHouse и создают SQLite таблицы. Никакие SQL-скрипты вручную запускать НЕ НУЖНО.
> - **Автосидинг RAG**: Агент `agent-rag` при запуске проверяет ChromaDB. Если векторная база пуста, он автоматически нарезает документацию из папки `documents/` на логические чанки, генерирует эмбеддинги `multilingual-e5-large` и сохраняет их в индекс.

Если вы хотите вручную проверить качество нарезки на чанки без записи в векторную базу, вы можете выполнить:
```bash
# Просмотр первых чанков в консоли без записи в ChromaDB:
